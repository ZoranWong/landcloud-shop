...
if (!function_exists('get_flow_no')) {
    function get_flow_no($type = 'order', $length = 6)
    {
        ...
        $available = count($flow_no_list);
        if ($available) {
            $index = mt_rand(0, $available - 1);
            $flow_no = $flow_no_list[$index];
            array_splice($flow_no_list, $index, 1);
...
//            $str = $type . substr(msectime() . rand(0, 9), 1);
            $str = ORDER_NO_PREFIX . date('Ymd', time()) . get_flow_no();
            break;
        case 2:         //支付单编号
            $str = $type . substr(msectime() . rand(0, 9), 1);
            break;
