...
class Categories extends Manage
{
    ...
    public function relateBrand($id)
    {
        ...
        if (!\think\facade\Request::isPost()) {
            ...
            $goodsCat = $goodsCatModel->with('brands')->findOrFail($id);
            $brands = $goodsCat->brands()->field('lc_brand.id')->select()->toArray();
            ...
            return $this->fetch('relate');
        } else {

        }
    }

    public function bindBrands($id)
    {
        $brands = input('brands');
        if (is_string($brands)) {
        ...
        if (\think\facade\Request::isPost()) {
            /** @var GoodsCatModel $goodsCat */
            $goodsCat = $goodsCatModel->findOrFail($id);

            $goodsCat->brands()->sync($brands, true);
    ...
    public function edit($id)
    {
        ...
        if (!Request::isPost()) {
            ...
            $this->assign('logo', $data['image_id'] ?_sImage($data['image_id']) : config('labgic.default_image'));
    ...
    public function getInfo()
    {
        ...
        $goodsCatModel = new GoodsCat();
        $cat = $goodsCatModel->with('brands')->field('id,name,sort,type_id')->where([['id', '=', $id]])->find();
        if ($cat) {
